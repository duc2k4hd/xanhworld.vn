@php
    $types = [
        \App\Models\Voucher::TYPE_FIXED => 'Giảm số tiền cố định',
        \App\Models\Voucher::TYPE_PERCENT => 'Giảm theo % đơn hàng',
            ...
            const loadProducts = async (page = 1, append = false) => {
                ...
                try {
                    @if(isset($voucher) && $voucher->id)
                        const productsUrl = `{{ route('admin.vouchers.products', $voucher) }}`;
                    @else
                        const productsUrl = null; // Không có voucher ID khi tạo mới
                    @endif

                    if (!productsUrl) {
                        console.warn('Voucher ID không tồn tại. Không thể tải danh sách sản phẩm.');
                        loading = false;
                        return;
                    }
                    ...
                    const response = await fetch(`${productsUrl}?${params}`);
                    const result = await response.json();
