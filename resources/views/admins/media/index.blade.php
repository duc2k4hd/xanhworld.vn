...
@push('scripts')
    <script>
        ...
        function handleUpload(files) {
            ...
            progressDiv.innerHTML = '<p>Đang upload...</p>';
            ...
            .then(r => {
                ...
                .then(data => {
                    if (data.success) {
                    const successCount = data.files ? data.files.filter(f => f.success).length : 0;
                    const totalCount = data.files ? data.files.length : 0;
                    progressDiv.innerHTML = `<p style="color:green;">Upload thành công ${successCount}/${totalCount} file!</p>`;
                    setTimeout(() => {
                    ...
                    progressDiv.innerHTML = `<p style="color:red;">${errorMsg}</p>`;
                }
            })
            .catch(err => {
                console.error('Upload error:', err);
                progressDiv.innerHTML = '<p style="color:red;">Lỗi khi upload</p>';
            });
        }
        ...
        document.getElementById('btnDeleteSelected').addEventListener('click', () => {
            ...
            const deletePromises = filesToDelete.map(path => {
                return fetch('{{ route("admin.media.delete") }}', {
                method: 'POST',
                headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
                ...
                .catch(err => {
                    console.error('Delete error for', path, ':', err);
                    errorCount++;
                });
            });
            ...
            Promise.all(deletePromises).then(() => {
                ...
                if (errorCount === 0) {
                    ...
                    alert(`Đã xóa ${deletedCount} file, ${errorCount} file lỗi!`);
                }
                });
            });
