                ...
                <tbody>
                    ...
                    <td>
                        <div style="display:flex;gap:4px;">
                            ...
                            <form action="{{ route('admin.flash-sales.items.delete', [$flashSale, $item]) }}" method="POST" style="display:inline;" onsubmit="return confirm('Xóa sản phẩm này khỏi Flash Sale?')">
                                @csrf
                                @method('DELETE')
    ...
    <div class="modal-content">
        ...
        <div class="tab-container">
            ...
            <div id="tab-content-manual" class="tab-content active">
                <form id="addProductForm" action="{{ route('admin.flash-sales.items.add', $flashSale) }}" method="POST">
                    @csrf
            ...
            <div id="tab-content-auto" class="tab-content">
                <form id="addProductByCategoriesForm" action="{{ route('admin.flash-sales.items.add-by-categories', $flashSale) }}" method="POST">
                    @csrf
...
<script>
    ...
    function loadPickerProducts(page = 1) {
        ...
        fetch(`{{ route('admin.flash-sales.items.products-by-category', $flashSale) }}?${params.toString()}`, {
            headers: {
                'Accept': 'application/json',
                'X-Requested-With': 'XMLHttpRequest',
            },
    ...
    window.loadSuggestedProducts = function (forceReload = false) {
        ...
        fetch(`{{ route('admin.flash-sales.items.suggest-best-selling', $flashSale) }}?limit=20`, {
            headers: {
                'Accept': 'application/json',
                'X-Requested-With': 'XMLHttpRequest',
            },
