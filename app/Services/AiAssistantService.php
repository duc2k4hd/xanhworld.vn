...
class AiAssistantService
{
    ...
    private function searchProducts(array $keywords): Collection
    {
        $query = Product::query()
            ->active()
            ->select(['id', 'name', 'slug', 'short_description', 'price', 'sale_price', 'stock_quantity', 'primary_category_id'])
    ...
    private function extractKeywords(string $question): array
    {
        ...
        return collect(preg_split('/\\s+/', (string) $normalized))
...
- Giữ văn phong thân thiện, súc tích và ưu tiên tiếng Việt.
- Đề xuất sản phẩm bằng cách nhắc tới tên và ưu điểm, không bịa đặt giá nếu không có thông tin.
PROMPT;
